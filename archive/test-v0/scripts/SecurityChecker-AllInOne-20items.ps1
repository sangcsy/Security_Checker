    ...
    try {
        switch ($ItemCode) {
            ...
            "W-09" {
                ...
                try {
                    ...
                    if (-not $isAdmin) {
                        # 관리자 권한 없으면 수동 확인
                        $status = "수동 확인 필요"
                        $currentState = "감사 정책 확인에 관리자 권한이 필요합니다. 수동으로 확인하세요."
                    } else {
                        # 관리자 권한으로 auditpol 실행
                        $auditResult = auditpol /get /category:* 2>&1
                        
                        if ($LASTEXITCODE -eq 0 -and $auditResult) {
                            # 로그온 관련 감사 확인
                            $logonLines = $auditResult | Select-String -Pattern "Logon|로그온" -Context 0,2
                            
                            if ($logonLines) {
                                $resultText = $logonLines -join " "
                                $hasSuccess = $resultText -match "Success|성공"
                                $hasFailure = $resultText -match "Failure|실패"
                                
                                if ($hasSuccess -and $hasFailure) { $status = "양호" }
                                elseif ($hasSuccess -or $hasFailure) { $status = "부분 양호" }
                                else { $status = "관리 필요" }
                                
                                $successText = if ($hasSuccess) { "활성화" } else { "비활성화" }
                                $failureText = if ($hasFailure) { "활성화" } else { "비활성화" }
                                $currentState = "로그온 감사 - 성공: $successText, 실패: $failureText"
                            } else {
                                $status = "수동 확인 필요"
                                $currentState = "로그온 감사 설정을 확인할 수 없음 (수동 확인 필요)"
                            }
                        } else {
                            $status = "수동 확인 필요"
                            $currentState = "감사 정책 조회 실패 (수동으로 확인하세요)"
                        }
                    }
                } catch {
                    $status = "수동 확인 필요"
                    $currentState = "감사 정책은 수동으로 확인해주세요"
                }
            }
            ...
            "W-11" {
                # 불필요한 공유 제거
                try {
                    # net share 명령어 사용 (모든 Windows 버전 호환)
                    $netShareResult = net share 2>$null
                    
                    if ($netShareResult) {
                        # 기본 관리 공유($로 끝나는 것) 제외
                        $shareLines = $netShareResult | Select-Object -Skip 3 | Where-Object { 
                            $_ -match '^\s*\S+' -and $_ -notmatch '명령을 잘 실행했습니다|The command completed' 
                        }
                        ...
                        if ($userShares.Count -gt 0) {
                            $shareList = $userShares -join ', '
                            $status = "수동 확인 필요"
                            $currentState = "공유 폴더 발견 ($($userShares.Count)개): $shareList"
                        } else {
                            $status = "양호"
                            $currentState = "사용자 정의 공유 폴더 없음 (기본 관리 공유만 존재)"
                        }
                    } else {
                        $status = "점검 불가"
                        $currentState = "공유 폴더 정보를 가져올 수 없음"
                    }
                } catch {
                    $status = "점검 불가"
                    $currentState = "공유 폴더 조회 중 오류 발생"
                }
            }
...
function Invoke-SecurityScan {
    ...
    foreach ($result in $results) {
        $statusTrimmed = $result.status.Trim()
        switch ($statusTrimmed) {
            "양호" { $summary.good++ }
            "관리 필요" { $summary.needs_management++ }
            "수동 확인 필요" { $summary.manual_check++ }
            "부분 양호" { $summary.partial_good++ }
            "점검 불가" { $summary.check_failed++ }
            default { 
                Write-Warning "알 수 없는 상태: [$statusTrimmed]"
